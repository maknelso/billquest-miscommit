...
class BillQuestMiscommitStack(Stack): # <<< NOTE: This class name MUST match your project's stack name
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Add GSI for efficient date-based queries - based on bill_period_start_date and product_code
        billing_data_table.add_global_secondary_index(
            index_name="DateProductIndex",
            partition_key=dynamodb.Attribute(
                name="bill_period_start_date", 
                type=dynamodb.AttributeType.STRING
            ),
            sort_key=dynamodb.Attribute(
                name="product_code", 
        ...
        # Add GSI for efficient invoice-based queries - based on invoice_id and product_code
        billing_data_table.add_global_secondary_index(
        ...
        raw_files_bucket = s3.Bucket(
            ...
            block_public_access=s3.BlockPublicAccess.BLOCK_ALL, # Highly recommended for security
            auto_delete_objects=False,
        )

        # --- 3. Lambda Function for S3 Ingestion (Reads from S3, writes to DynamoDB) ---
        # This function will be triggered when a new file is uploaded to raw_files_bucket.
        ingest_lambda = lambda_.Function(
            self, "IngestDataLambda",
            function_name=f"{stack_prefix}-IngestDataLambda-nelmak",
            runtime=lambda_.Runtime.PYTHON_3_10, # Specify the Python runtime version
            handler="app.lambda_handler", # Points to 'app.py' and 'lambda_handler' function within the 'lambda/ingest_data' directory
            code=lambda_.Code.from_asset("lambda/ingest_data"), # Path to your Lambda code directory, relative to your CDK app root
            timeout=Duration.seconds(30), # Max execution time for the Lambda. Adjust based on expected file size/processing.
            environment={ # Environment variables for the Lambda function
                "TABLE_NAME": billing_data_table.table_name # Pass the DynamoDB table name
            }
        )

        # Grant the Ingest Lambda necessary permissions
        raw_files_bucket.grant_read_write(ingest_lambda) # Allow Lambda to read and write to S3 bucket
        billing_data_table.grant_write_data(ingest_lambda) # Allow Lambda to write to the DynamoDB table
        ...
        query_lambda = lambda_.Function(
            self, "QueryDataLambda",
            function_name=f"{stack_prefix}-QueryDataLambda-nelmak",
            runtime=lambda_.Runtime.PYTHON_3_10,
            handler="app.lambda_handler", # Points to 'app.py' and 'lambda_handler' in 'lambda/query_data'
            code=lambda_.Code.from_asset("lambda/query_data"), # Path to your Lambda code directory, relative to your CDK app root
            timeout=Duration.seconds(10), # Max execution time. Adjust as needed.
            environment={
                "TABLE_NAME": billing_data_table.table_name # Pass the DynamoDB table name
            }
        )
        ...
        billing_data_table.grant_read_data(query_lambda) # Allow Lambda to read from DynamoDB
        ...
        query_resource.add_method(
            ...
            apigw.LambdaIntegration(query_lambda), # Connects the API Gateway method to your Lambda function
            api_key_required=False # Set to True for production to control access with API Keys
        )
        ...
        distribution = cloudfront.Distribution(
            ...
            default_behavior=cloudfront.BehaviorOptions(
                ...
                viewer_protocol_policy=cloudfront.ViewerProtocolPolicy.REDIRECT_TO_HTTPS # Forces HTTPS
            ),
            default_root_object="index.html" # CloudFront will serve index.html for root requests
        )
        ...
        s3deploy.BucketDeployment(
            self, "DeployWebsite",
            sources=[s3deploy.Source.asset("./frontend/dist")], # Source directory for your built frontend
            destination_bucket=website_bucket,
            distribution=distribution, # Triggers CloudFront cache invalidation after deployment
            distribution_paths=["/*"] # Invalidate all paths to ensure fresh content
        )
